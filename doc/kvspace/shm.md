...
> 前置：DSN 机制（issue/fix-012，`--kvspace scheme://addr`）、kvspace-rdma-distributed.md（多组件共享数据平面）、kvspace-rdma 仓库根设计（RDMA server 行业对标）
...
| 目标 | 含义 |
|------|------|
| **语义等价** | 12 方法语义以 `internal/kvspace/kvspace.go` 接口注释 + redis 后端行为为锚，tutorial 全量作为跨后端一致性测试 |
...
| 层 | 内容 | 归属 |
|----|------|------|
| KVSpace 接口 | 12 方法契约 | `kvlang/internal/kvspace`（权威） |
