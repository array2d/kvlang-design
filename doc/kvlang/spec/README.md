...
> kvlang 是 kvlang 元程级的编程语言。**kvir 即 kvlang 的指令设计部分**——两者不是两层，而是同一语言的不同视角。

## 0. 统一语法哲学

LLVM 将表示划分为多层：高级语言 → 中层 IR → 低级机器码，各层语法互不相通，编译器在其中逐层翻译。

kvlang 不这样做。**kvlang 以同一种语法，同时承担四种职能**：
...
kvlang:  def  →  op(A,B)->C  →  /vthread/[i,j]
        同一语法，同一表示，视角不同而已
```

**kvir** 即这条指令 `op(A, B) -> C` 的设计——它不是独立的一层，它就是 kvlang 本身，是 kvlang 在"指令"视角下的名字。
...
这就是 kvlang 的核心设计意图：**为 AI 任务服务的声明式语言**——数据流用箭头 (`->`) 表达，计算交给平台代理，使用者只需关心"什么算子、什么数据、什么顺序"，无需关心 GPU 型号、内存布局、进程间通信。

与 C/CUDA 的本质差异：

```
C/CUDA:  程序员管理一切 ── 内存分配、设备选择、数据传输、kernel launch
kvlang:  程序员只写数据流 ── 底座自动调度、分配、执行、回收
```

kvlang 不接触物理地址，不管理内存分配，不感知设备拓扑——这些全部由 heap-plat 和 op-plat 的常驻进程代理。
...
### 只读/写入参数标记
kvlang 支持`<-` 与 `->`（或者`<=` 与 `=>`），用于显式区分只读与写入参数，箭头指向写入参数列表；
...
```
# 正确：字符串用双引号，Key 用单引号
newtensor("f32", "[128]") -> '/data/a'       # "f32"=dtype字符串, '/data/a'=key路径
print("result is", './x')                     # "result is"=字符串, './x'=key读值
```

```
# 错误示例（解析器拒绝）
newtensor(f32, [128]) -> /data/a              # key /data/a 缺少单引号 → parse error
newtensor('f32', '[128]') -> "/data/a"        # 语义反转：f32不是key，/data/a不是字符串
```

### 设计动机

- Key 路径是分布式 KV 空间的一等公民，单引号使其在源码中视觉突出
- 字符串字面量（dtype、shape、错误消息）是普通数据，双引号遵循通用编程语言习惯
- 避免歧义：`"f32"` 是字符串 `f32`，`'f32'` 是名为 `f32` 的 key；两者可能指向不同数据
...
## 6. 设计思考
kvlang 采用简洁文本格式表达类型约束、运算定义与运算体，便于阅读与解析。  
kvlang 不是 SSA，调用时遵循一侧读、另一侧写的规则，参数列表支持多个。  
kvlang 作为调度语义与协议载体，不负责算子实现与存储生命周期。  
