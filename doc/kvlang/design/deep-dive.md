...
1. **读写码**（kvcpu 直接执行）：`writes <- opcode(reads...)` 或 `opcode(reads...) -> writes`。读参写参由箭头方向决定，无隐式栈、无匿名寄存器。存储布局见 §2 二维空间模型。
...
| **kvspace** | `internal/kvspace/` | KV 存储接口：Get/Set/Del/DelTree/List/Watch/Notify/Link |
...
**铁律**：`->` 右侧必须是位置（指针）；字面量（数字、引号串）在写槽位置是语法错误，Parser 拒绝并报警。
...
| kvspace 抽象存储 | 存储后端可替换，接口仅 Get/Set/Del/GetMany/SetMany/List/DelTree/Notify/Watch/Link/Unlink/DisConn |
