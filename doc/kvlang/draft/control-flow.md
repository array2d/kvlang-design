...
## 1. 四层对比：Assembly IR、kvir、MLIR、C 语言

> **MLIR** (Multi-Level Intermediate Representation) 与 kvir 的设计目标高度重合：
> 都面向深度学习计算场景，都需要在计算图和指令级之间提供可组合的中间表示。
> kvir 可以视为 MLIR 在 **KV 存储原生、分布式调度、多后端异构** 方向上的一个垂直领域方言实现。
...
MLIR 的三个核心设计对 kvir 有直接参考价值：

#### Dialect 方言体系 vs kvir 算子分类
...
MLIR:                              kvir (类比):
  ...
  gpu dialect  (GPU 抽象)            op-<backend> ← 后端调度（动态，如 op-cuda / op-pytorch / op-jax）
  memref dialect (内存抽象)           newtensor/deltensor ← 生命周期层
  arith dialect (算术)               + - * / % + 原生算子 ← 求值层
```

MLIR 的 **dialect 混用** 允许在同一函数中混合不同抽象层的操作（如在 `scf.for` 循环内调用 `linalg.matmul`），kvir 当前已经隐式实现了类似能力——生命周期 (`newtensor`)、计算 (`add`)、控制 (`if`) 可以在同一函数体中混用。**但 kvir 缺少 dialect 的显式声明和校验机制**。
...
```
MLIR 编译流程:                    kvir 当前 + 目标流程:
                               
  前端 (C++/PyTorch)      →       pysdk 写入 /src/func/ (kvlang)
    │                                │
  dialect lowering          →       ParseKvlang → ir.Instruction (语法→结构)
    │                                │
...
kvir 的当前流程已经是一条隐式的 "pass pipeline"，但缺少 MLIR 的 **可组合性** 和 **可验证性**：每个 pass 的输入输出格式没有形式化定义，变换的正确性依赖人工保证而非结构化约束。
...
call  → translate.HandleCall()
  - eager inline: 一次性将编译层 kvlang 翻译为执行层 [i,j] 坐标
