# Kvlang 控制流与前端代码生成 — 架构分析 v1

> 方案 C 确定为最优架构方向。本文展开 5 种子方案 + kvir 语法完整设计。
...
| 约束 | 原因 |
|------|------|
| **保留 `->` 和 `<-`** | 显式区分只读参数和写入参数方向，是 kvir 的协议级语义 |
...
kvir 不是通用编程语言。它的指令格式（`reads` 数组 + `writes` 数组）天然有方向性。
箭头直接表达这个方向：
...
kvir 的核心场景——GPU 显存分配/释放、分布式 KV 读写、多后端异构执行——**本质上就是副作用密集的**。
因此 `->`/`<-` 是更自然的匹配。
...
### 4.9 为什么 kvir 选择 `->`/`<-` 作为主语法而非 SSA

| 决策因素 | `->`/`<-` | SSA | 分析 |
|---------|-----------|-----|------|
| **协议对齐** | 直接映射到 reads[]/writes[] | 需要额外 lowering | kvir 的协议层就是以 reads/writes 为单位的, 强行用 SSA 增加无谓转换 |
...
| **分布式语义** | 天然 (路径即全局标识) | 需额外映射 (值编号是局部的) | kvir 是分布式系统, KV 路径就是全局标识 |
| **副作用表达** | 自然 (new/del 就是指令) | 需要 dialect 隔离 | 不需要引入 dialect 概念, 降低系统复杂度 |
| **优化能力** | 需额外分析 | 天然支持 | 但 kvir 当前优化需求不高 (计算在 GPU 上), 且 C2 Scheduler 内部可以重建 SSA |

**结论: `->`/`<-` 是 kvir 的正确主语法选择。SSA 作为 C2 Scheduler 内部的优化表示, 在 lowering 阶段构建, 对前端和 VM 透明。**
...
前后端共用 kvir 语法。VM 直接解释控制流关键字，管理嵌套作用域。
...
**C1 与 C2 互补**：
1. **先 C1**：kvir 语法 → VM 直接解释，调试友好，快速可用
