...
调用时（HandleCall）：
  ...
  7. 写 .callpc（返回地址）、.rootfunc（函数名）、.ro（读参名单，fix-027）
  8. 返回 frameRoot+"/.funclib/[0,0]"                    # 子帧第一条指令 PC

返回时（HandleReturn）：
  1. 读 .callpc → 恢复父 PC；写参已在子帧执行期间经 .wparam 直写父帧，无需搬运
  2. kv.Unlink(frameRoot+"/.funclib")                     # 摘链
  3. kv.DelTree(frameRoot)                          # 清整个子帧（params/.rparam/.wparam/.callpc/.rootfunc）
  4. 返回 op.NextPC(callPC)                          # 父帧下一条
...
| 键 | 机制 | 代码位置 | 语义 |
|----|------|---------|------|
| `.funclib` | 软链接（Link） | `FuncLib` / `HandleCall:Link` / `HandleReturn:Unlink` | → `/lib/<pkg>.<name>` 只读指令区；TCO 时 Unlink + 重链；**PC 格式依赖**：`FrameRoot`/`ChildFrameRoot` 以 `"/.funclib/"` 定位帧边界 |
