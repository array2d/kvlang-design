...
栈路径（帧根）不需要单独的寄存器——PC 本身是帧内路径（帧的代码区通过帧根的 .funclib 软链访问），`FrameRoot(PC)` 截取即得。这与 C 的 `rbp + offset` 同构：
...
```
/vthread/run/               ← 调用方帧根
/vthread/run/（经 .funclib Link）           ← 调用方代码区（软链接）
...
| 可观测 | 需调试器 attach | `kvspace dump /vthread/…` 看 `.funclib` 链向谁、frameRoot 在哪个坐标 |
...
| 键 | 机制 | 代码位置 | 语义 |
|----|------|---------|------|
| `.funclib` | 软链接（Link） | `FuncLib` / `HandleCall:Link` / `HandleReturn:Unlink` | → `/lib/<pkg>.<name>` 只读指令区；kvcpu 以 `FuncLib(FrameRoot(pc))` 为 keyBase 取指 |
...
3. **与帧系统键统一为一条公理**。帧本就是对象，`frameRoot/.pc`、`.funclib`、`.callpc` 已是 `{对象}/.var` 形态。公理：**任何 kvspace 对象 X——VM 元数据在 `X/.名`，用户数据在 `X.名`，结构子级在 `X/名`**。帧的 `.funclib`、`.rootfunc`、`.ro` 已是 `{对象}/.var` 形态。数组 `.shape`、未来 GC 计数器直接套用，无需新机制。
