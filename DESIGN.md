...
kvlang 的主张：**程序 = 数据结构 + 函数 + 数据**。数据结构不再由用户自定义——kvlang 的全部数据结构都是 builtin：`struct ≡ dict`（键族前缀），`array`（XValue 内 `arraylength` 定长同构数组，元素按 `kind` 定长偏移索引），`link list`（`/n0.val`、`/n0.next` 平坦键，`"/n1"` 存路径字符串即指针）。agent 编程时不需要"设计数据结构"——只需要声明类型（`lib name { }`）和写函数（`def`），数据在 kvspace 四域（`/lib/` `/vthread/` `/sys/` `/dev/`）里自然落地。
...
- `/vthread/` 是运行时栈帧，借用 Unix `/proc/<pid>/` 思想——每 vthread 一棵子树，`.pc`/`.status`/`.funclib` 等系统键暴露执行状态
...
例：PC = /vthread/7/[3,0]/.funclib/[1,0]
    FrameRoot(PC) = /vthread/7/[3,0]      ← 截断最后一个 /.funclib/
    x → /vthread/7/[3,0]/x
```

栈路径（帧根）不需要单独的寄存器——PC 本身是帧内路径（帧的代码区在 `帧根/.funclib/` 下），`FrameRoot(PC)` 截取即得。这与 C 的 `rbp + offset` 同构：
...
```
/vthread/run/               ← 调用方帧根
/vthread/run/.funclib/           ← 调用方代码区（软链接）
...
顶层调用（Bootstrap）：无 .callpc，直接 Link funcKey → vthreadRoot/.funclib。
```
...
| TCO | 需特殊优化（复用帧 + 重定向参数） | Unlink + Link，已有的 .funclib 机制天然支持 |
| 崩溃恢复 | 栈帧在内存，进程死即全失 | PC=路径字符串、`.callpc`=返回点落 KV——重启续跑 |
| 可观测 | 需调试器 attach | `kvspace dump /vthread/…` 看 `.funclib` 链向谁、`.callpc` 在哪个坐标 |
...
```go
type XValue struct {
    kind        string // vtype name（int/float/bool/string/bytes/tensor）
    arraylength int32  // 数组长度，单值=1，>1 即数组
    raw         []byte // 类型化原始字节：kindSize(kind) * arraylength 字节
}
```

`IsNil()`（kind==""）即 null。`dict` 是零负载类型标记（§10.3）。

**TLV 线格式**：`[1B kind_len][N B kind_name][4B arraylength LE][4B raw_len LE][M B raw_value]`。单值 arraylength=1；数组 arraylength=N，raw 连续存放 N 个元素。IsNil() 编码为零字节。

**1D 数组**：数组性由 `arraylength > 1` 判定，无独立的 "array" kind。kind 必须为基础定长类型（int/float/bool 等），元素在 raw 中连续存储。索引通过指针偏移：`off = i * kindSize(kind)`。变长类型（string/bytes）不作为 XValue 数组元素——使用 kvspace 键族存储。

**kindSize 表**（定长类型字节数）：bool/int8/uint8→1，int16/uint16→2，int32/uint32/float32→4，int64/uint64/float64/float/int→8。其余→0（不变长类型）。

**语法**：
```kv
a:int = 1                   # 单值变量声明（写槽 : 类型标注）
arr:int = [1, 2, 3]         # 同构定长数组，= 等价于 <-
[1, 2, 3] -> arr:int        # 等价写法（写槽在右）
```
...
| 数组：`a[i]`（XValue 1D 定长） | kindSize×i 指针偏移；`a[i]` → `at(a, i)`。kvspace 键族数组另见键族 |
...
| `.debugger` | String | 调试控制：`""` 正常，`"step"` 单步；`debugger()` builtin（fix-031，对齐 V8/TypeScript `debugger;`）读此键 |
| `.debugger.pause` | Notify 队列 | CPU → agent 暂停事件（JSON） |
| `.debugger.resume` | Notify 队列 | agent → CPU 恢复命令：`step`/`continue`/`abort` |

**帧对象**（宿主 = frameRoot，调用协议；vthread 根即顶层帧）：

| 键 | 机制 | 语义 |
|----|------|------|
| `.funclib` | 软链接 | → `/lib/<pkg>.<name>` 只读指令区；TCO 时 Unlink + 重链 |
| `.callpc` | String | 调用指令绝对地址；HandleReturn 据此推算写槽路径并恢复父 PC（仅子帧） |
| `.rootfunc` | String | 帧对应函数名；TCO 复用帧时不更新（顶层帧由 Bootstrap 写入） |
| `.ro` | String | 只读参数名单（逗号分隔），kvcpu 写槽检查用（fix-027；无参函数不写） |
...
**语法层保留名（不落盘）**：`._` 丢弃槽——`frameSlotKey` 对 `.xxx` 槽返回空路径直接忽略。
...
| `X/.名`（`/` + dot 名） | `/vthread/7/.pc` | 系统变量（影子元数据） | VM |
...
3. **与帧系统键统一为一条公理**。帧本就是对象，`frameRoot/.pc`、`.funclib`、`.callpc` 已是 `{对象}/.var` 形态。公理：**任何 kvspace 对象 X——VM 元数据在 `X/.名`，用户数据在 `X.名`，结构子级在 `X/名`**。数组 `.shape`、未来 GC 计数器直接套用，无需新机制。
